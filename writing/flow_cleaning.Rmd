...
Note that I only pulled the "Control" samples because they're unstimulated

Looking at CD3 - 
Pig 3273_Surface_Control_016.fcs
Pig 4754_Surface_Control_028.fcs
_016 and _028 are off based on manual gating (check initial lymph, singlet, etc.), but the other are pretty good

Things to do:
 - Figure out CD3 gating
...
**Looks like CD3 is the only potential problem right now..**
```{r}
...
corrected_data <- saved_data %>%
  mutate(CD172_break = ifelse(CD172_break <2, NA, CD172_break),
         CD45RA_break = ifelse(CD45RA_break >3, NA, CD45RA_break),
         CD4_break = ifelse(CD4_break >3, NA, CD4_break),
         CD3_break = ifelse(CD3_break <1, NA, CD3_break)) %>%
  mutate(CD3_break = 2.2,
         CD4_break = mean(CD4_break, na.rm = TRUE),
         CD8_break = mean(CD8_break, na.rm = TRUE),
         CD45RA_break = mean(CD45RA_break, na.rm = TRUE),
         CD172_break = mean(CD172_break, na.rm = TRUE),
...
all_data_breaks %>%
  select(filename, CD3_break, CD3) %>%
  group_by(filename) %>%
  mutate(cells = 1) %>%
  mutate(total_cells = sum(cells)) %>%
  filter(CD3>CD3_break) %>%
  mutate(CD3_percentage = 100*sum(cells)/total_cells) %>%
    select(filename, CD3_percentage) %>%
    unique()
...
total_phenotypes <- filter_for_total_pheno(all_fe_12week, marker_order = order_of_markers)
...
sample_populations <- all_fe_12week %>%
  dplyr::filter(CD3 == 1 & percentage > 1) %>%
  arrange(CD3, CD4, CD8) %>%
  filter_pops() 

sample_populations_all_groups <- identified_pop_perc(sample_populations, all_fe_12week, 
