...
Understandig linear regressions with interactions
...
Check CSU library for lienar regression - Springer
```{r}
...
do.call(rbind, ajk) %>%
  ...
  filter(change <=2 & change >= 0.5) # it's okay that we're removing values with negative change because we're moving those that have opposing relationships
...
# Final PCA - July 20
```{r}
# this gives us 25 populations to look at more closely - note that I think I tested the trends on the opposite lm than the original 
minipig_PCA %>%
  group_by(PC, flow) %>%
  nest() %>%
  mutate(model = map(data, ~lm(flow_percent ~ PC_value *timepoint, data = .)),
...
# then pull out coefficient - between 0.5 and 2

# timepoint doesn't have an effect - only include correlations that aren't found in interaction model 
minipig_PCA %>%
  group_by(PC, flow) %>%
  nest() %>%
  mutate(model = map(data, ~lm(flow_percent ~ PC_value +timepoint, data = .)),
...
fm1 <- df %>%
  filter(flow == flow_pop & PC == metab_pick) %>%
  lm(formula= flow_percent ~ PC_value *timepoint, data=.)
...
plot_complex(minipig_PCA, "CD45RA", "PC4", 75, 5, 10) 
plot_complex(minipig_PCA, "CD8_Tcell", "PC4", 75, 5, 10) 
plot_complex(minipig_PCA, "Pop16", "PC4", 75, 5, 10) 
plot_complex(minipig_PCA, "Pop2", "PC7", 75, 5, 10) 
plot_complex(minipig_PCA, "Pop24", "PC4", 75, 5, 10) 

plot_complex(minipig_PCA, "Pop11", "PC6", 75, 5, 10) 
...
Final analysis if I flip PC and flow 
```{r}
# gives us 38 populations when we only look at pvalue <0.05 for PC_value
pops_list <- minipig_PCA %>%
  group_by(PC, flow) %>%
  nest() %>%
  mutate(model = map(data, ~lm(PC_value ~ flow_percent *timepoint, data = .)),
         summary_model = map(model, tidy)) %>%
  unnest(summary_model) %>%
  select(flow, PC, term, p.value) %>%
  filter(term == "flow_percent") %>%
  filter(p.value < 0.05) %>%
  arrange(flow, PC) 

# we take this list of 38 flow-PC populations and test for difference between 12 wks and 30DPI timepoint
cell <- list(flow_x = pops_list$flow, PC_n = pops_list$PC)
...
# Test the best clustering model
```{r}
library(AICcmodavg)
ii <- minipig_PCA %>%
  group_by(PC, flow) %>%
  nest() %>%
  mutate(model1 = map(data, ~lm(flow_percent ~ PC_value * timepoint, data = .)),
         summary_model1 = map(model1, summary),
         model2 = map(data, ~lm(flow_percent ~ PC_value, data = .)),
         summary_model2 = map(model2, summary)) 

model.set <- list(ii$model1, ii$model2)
model.names <- c("lm1", "lm2")
comp <- map(model.set, aictab)

comp1 <- comp[1] %>%
  as.data.frame() %>%
  select(Modnames, AICc) %>%
  rename(inter_AICc = AICc)

comp2 <- comp[2]%>%
  as.data.frame()%>%
  select(Modnames, AICc)%>%
  rename(linear_AICc = AICc)

full_join(comp1, comp2) %>%
  mutate(inter = ifelse(inter_AICc < linear_AICc, "inter", "linear")) %>%
  count(inter)

############### see if differences if change my PC and flow
ii <- minipig_PCA %>%
  group_by(PC, flow) %>%
  nest() %>%
  mutate(model1 = map(data, ~lm(PC_value ~ flow_percent * timepoint, data = .)),
         summary_model1 = map(model1, summary),
         model2 = map(data, ~lm(PC_value ~ flow_percent, data = .)),
         summary_model2 = map(model2, summary)) 
...
For each metabolite, tests for normality with shapiro wilks, if not, then runs a non-parametric test

0 PCA - Ask Bailey about testing for normality - could also speak with zaid
Are there non-parametric tests for the interaction
```{r}
hist(metabolites$metabolite_expression)
hist(flow_fe$percentage)

shapiro.test(flow_fe$percentage)
shapiro.test(metabolites$metabolite_expression)
```
