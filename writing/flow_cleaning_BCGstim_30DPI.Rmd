---
title: "flow_cleaning_BCGstim_30DPI"
...
fcsFiles <- list.files("../data/BCG_Stim/Surface_30_DPI", full = TRUE)
fcsFiles
...
corrected_data <- saved_data %>%
  mutate(CD4_break = ifelse(CD4_break >3, NA, CD4_break),
         ...
         SLADQ_break = ifelse(SLADQ_break <2.5 | SLADQ_break >4, NA, SLADQ_break)) %>%
  mutate(CD3_break = 2.3,
...
unique(all_data_breaks$filename)
# check the gating with the percentages


all_data_breaks %>%
  select(filename, CD4_break, CD4) %>%
  group_by(filename) %>%
  mutate(cells = 1) %>%
  mutate(total_cells = sum(cells)) %>%
  filter(CD4>CD4_break) %>%
  mutate(interested_percentage = 100*sum(cells)/total_cells) %>%
    select(filename, interested_percentage) %>%
    unique()
```
...
all_fe_BCG_30DPI <- all_data_breaks %>%
...
save(all_fe_BCG_30DPI, file = "../data/all_fe_BCG_30DPI")
```
...
total_phenotypes <- filter_for_total_pheno(all_fe_BCG_30DPI, marker_order = order_of_markers)
...
sample_populations <- all_fe_BCG_30DPI %>%
  dplyr::filter(CD3 == 1 & percentage > 1) %>%
  arrange(CD3, CD4, CD8) %>%
  filter_pops() 

sample_populations_all_groups <- identified_pop_perc(sample_populations, all_fe_BCG_30DPI, 
...
save_for_phenotypes <- all_fe_BCG_30DPI %>%
