...
single_expression %>%
  filter(expression == "1") %>%
  group_by(timepoint, marker) %>%
  tukey_hsd(percent_per_marker ~ vaccine_status) 
...
ggplot(all_phenotypes, aes(x = factor(timepoint, levels = c("Week12", "D30DPI")), 
...
all_phenotypes %>%
  group_by(phenotype) %>%
  tukey_hsd(marker_percent ~ timepoint) 
```
...
sample_populations_all_groups %>%
  ...
  ungroup() %>%
  arrange(population, paste0("Pop", c(1:length(adj.p.value))))
```
...
calculate_plot_r_labeling <- function(important_metab, df) {
...
flow_metab_corr_plot <- function(df, important_metab, markerID, r_labels) {
  df %>%
  filter(marker == markerID) %>%
