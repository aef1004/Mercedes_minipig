...
load("../data/all_fe_30DPI.gz")
load("../data/all_fe_12week.gz")
...
single_expression %>%
  filter(expression == "1") %>%
  ggplot(aes(x =factor(timepoint, levels = c("Week12", "D30")), shape = factor(vaccine_status), y = percent_per_marker, color = timepoint)) +
  geom_point() +
  facet_wrap(~marker) +
...
all_phenotypes %>%
  ...
  ggplot(aes(x = factor(timepoint, levels = c("Week12", "D30DPI")), 
...
ggplot(sample_populations_all_groups, aes(x = factor(timepoint, levels = c("Week12", "D30DPI")), y = percentage, 
...
ggplot(pops_for_average_timepoint, aes(x = timepoint, y = average_percent, 
                                   group = timepoint, color = timepoint)) +
  scale_fill_identity() +
  geom_bar(stat = "identity") +
  facet_wrap("population", scales = "free",  labeller = label_both) +
