...
ggplot(aes(x = percent, y = metabolite_expression, color = factor(timepoint), group = timepoint)) +
  ...
  geom_text(data = rlabel, aes(x = 20, y = 60000,
    ...
    geom_text(data = rlabel, aes(x = 20, y = 50000,
              ...
              color = "#F8766D",
              parse = FALSE) +
  ggtitle("Metabolite 4363") +
  labs(color = "Timepoint", x = "CD8 Tcell Percentage of Cells", y = "Metabolite Expression", fill = "Timepoint") +
...
flow_list <- read.csv("../data/saved_data/Nurul_metabolites_by_flow_pop.csv") %>%
  select(flow) %>%
  unique() %>%
  as.vector()
...
data.frame(flow = df_fitted_models$flow, fdr, pval_alpha = pval_alpha$pval_alpha) %>%
  unique()
...
map_df(list_of_models, function(x) fdr(x)) %>%
  ...
  arrange(flow)  %>%
  select(flow, pval_alpha, fdr) %>%
  rename("Flow" = flow,
         "False Discovery Number" = fdr,
         "Number P-values <= 0.01" = pval_alpha) %>%
  kable(align = "l", caption = "Summary of P-value Historam False Discovery Rate") %>%
